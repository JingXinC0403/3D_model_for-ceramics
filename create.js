...
(function () {
  ...
  async function init() {
    ...
    try {
      ...
      // Keep older Google Sheets records compatible with the current editor.
      project.cameras = {
      ...
      const currentUser = CareAuth.cachedUser();
      if (dashboardUserEl && currentUser) {
        dashboardUserEl.textContent = currentUser.name || currentUser.email || "Account";
      }
  ...
  async function handleSubmit() {
    ...
    // Create/Save button writes to Google Sheets. This prevents a pending
    ...
    try {
      ...
      // Only leave the page after Google Sheets confirms the write succeeded.
      window.location.href = "index.html";
    } catch (error) {
      ...
      alert(
        ...
        "Make sure you are logged in, Google Sheets is enabled, and its rules allow signed-in users to write to the artefacts collection."
      );
    }
  }
