/* ============================================================
   boot.js
   ------------------------------------------------------------
   Load this FIRST, before the Firebase CDN scripts, on every
   page that depends on Firebase (index.html, create.html,
   project.html, login.html).

   Problem this solves: if Firebase fails to load or initialise
   ...
   listens for load/runtime errors on the Firebase scripts, and —
...
(function () {
  ...
  function buildOverlay() {
    ...
    overlay.innerHTML =
      ...
      "<li>Disable ad-blockers or privacy extensions for this site — they sometimes block Firebase/Google scripts.</li>" +
      "<li>Confirm Firestore Database is created (not just Authentication) in the Firebase console for this project.</li>" +
  ...
  var timer = setTimeout(function () {
    if (dismissed) return;
    showFatal(
      "This is taking much longer than expected, which usually means Firebase never finished connecting.",
      fatalReasons.length ? fatalReasons.join("\n") : "No error was reported — this can happen when Firestore Database hasn't been created yet in the Firebase console."
    );
  }, TIMEOUT_MS);
